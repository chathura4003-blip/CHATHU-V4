...
async function showQualityMenu(sock, from, meta, sender, ownerRefs = []) {
  ...
  // Also send the bot-wide button/list quality menu so users with
  // buttons-capable WhatsApp clients can tap a row instead of (or in
  // addition to) replying with a number. Best-effort: any failure here
  // never breaks the legacy numeric flow above.
  try {
    await ui.sendQualityMenu(sock, from, meta, { send: { quoted: meta.msg || null } }, { sender, ownerRefs, chatJid: from });
  } catch (_) {}
}
