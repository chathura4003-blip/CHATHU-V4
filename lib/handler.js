...
function adaptiveMaxWords(userText, userMax) {
    ...
    if (len < 120) return Math.min(userMax, userMax);
    return userMax;
}
...
function matchesWakeTrigger(cleanText, botName, botNumber, wakeWords) {
    ...
    if (firstWord && firstWord.length > 2 && new RegExp(`\\b${firstWord}\\b`).test(cleanText)) return true;
    for (const w of (wakeWords || [])) {
        const wl = String(w || '').toLowerCase().trim();
        if (!wl) continue;
        if (wl.length <= 3) {
            if (new RegExp(`(?:^|[^a-z0-9])${wl}(?:$|[^a-z0-9])`, 'i').test(cleanText)) return true;
