...
async function sendListMenu(sock, jid, menu, items, options = {}) {
  ...
  const rows = items.map((item, index) => ({
    title:       cleanText(item.title || item.label || `Option ${index + 1}`).slice(0, 60),
    rowId:       item.action || item.id || `menu:item:${index}`,
    description: cleanText(item.description || item.subtitle || "").replace(/https?:\/\/\S+/g, "").trim().slice(0, 70),
  }));
  const content = {
    text:        menu.text || menu.body || "Please choose one option.",
    footer:      menu.footer || "CHATHU MD",
...
async function sendMenu(sock, jid, menu, options = {}, context = {}) {
  ...
  logger(`[sendMenu] buttonMode=${buttonMode} menu=${menu.id} items=${items.length} attempting`);
  ...
  try {
    if (buttonMode === "off") {
      ...
      try {
        if (items.length <= 3) {
          logger(`[sendMenu] <=3 items → trying sendButtonMenu`);
          sent = await sendButtonMenu(sock, jid, renderedMenu, items, options);
        } else {
          logger(`[sendMenu] >3 items → trying native-flow sendListMenu`);
          try {
            sent = await sendListMenu(sock, jid, renderedMenu, items, options);
          } catch (listErr) {
            logger(`[sendMenu] native-flow list failed → trying sendClassicListMenu: ${listErr.message}`);
            sent = await sendClassicListMenu(sock, jid, renderedMenu, items, options);
          }
        }
      } catch (e) {
        logger(`[sendMenu] all rich sends failed → advanced text fallback: ${e.message}`);
        sent = await sendTextMenu(sock, jid, fallbackText, options);
      }
    }
  } catch (err) {
