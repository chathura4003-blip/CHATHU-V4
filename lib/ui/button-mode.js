 ...
 *
 * Final behavior (mirrors the wabase-button reference repo):
 ...
 *         If interactiveButtons fails: send advanced text fallback.
 *         If the current Baileys silently ignores `interactiveButtons` (which
 *         @whiskeysockets/baileys 6.x does), we emit a native-flow
 *         InteractiveMessage via relayMessage so users still see real buttons.
...
async function sendButtonMenu(sock, jid, menu, items, options = {}) {
  ...
  if (_baileysSilentlyDropped(sent)) {
    logger("[UI] interactiveButtons silently ignored by current Baileys; using native-flow relayMessage fallback");
    return await _sendNativeFlowQuickReply(sock, jid, menu, buttons, options);
  }
...
async function sendListMenu(sock, jid, menu, items, options = {}) {
  ...
  if (_baileysSilentlyDropped(sent)) {
    logger("[UI] interactiveButtons silently ignored by current Baileys; using native-flow relayMessage fallback");
    return await _sendNativeFlowList(sock, jid, menu, [singleSelect], options);
  }
...
// ---------------------------------------------------------------------------
// Native-flow relayMessage fallback (option 1 in the task description).
//
// @whiskeysockets/baileys 6.x does not understand the top-level
// `interactiveButtons` field — it silently builds an `extendedTextMessage`,
// so users only see the body and not the actual buttons. We detect that case
// and re-send the same payload as a proto `InteractiveMessage` via
// `relayMessage`, which is the same approach the legacy role-menu uses.
// ---------------------------------------------------------------------------

function _baileysSilentlyDropped(sent) {
  if (!sent || !sent.message) return false;
  const m = sent.message;
  if (m.interactiveMessage) return false;
  if (m.viewOnceMessage?.message?.interactiveMessage) return false;
  if (m.viewOnceMessageV2?.message?.interactiveMessage) return false;
  // If we asked for interactive buttons and got back an extendedTextMessage,
  // Baileys ignored the field.
  return Boolean(m.extendedTextMessage || m.conversation);
}

async function _sendNativeFlowQuickReply(sock, jid, menu, buttons, options = {}) {
  const baileys = require("@whiskeysockets/baileys");
  if (typeof sock?.relayMessage !== "function" ||
      typeof baileys.generateWAMessageFromContent !== "function") {
    throw new Error("native-flow relayMessage unavailable");
  }

  const interactive = baileys.proto.Message.InteractiveMessage.fromObject({
    body: { text: cleanText(menu.text || menu.body || "Choose an option from below", 1024) },
    footer: { text: cleanText(menu.footer || "CHATHU MD", 60) },
    header: {
      title: cleanText(menu.title || "CHATHU MD V4", 60),
...
async function _sendNativeFlowList(sock, jid, menu, buttons, options = {}) {
  const baileys = require("@whiskeysockets/baileys");
  if (typeof sock?.relayMessage !== "function" ||
      typeof baileys.generateWAMessageFromContent !== "function") {
    throw new Error("native-flow relayMessage unavailable");
  ...
  const interactive = baileys.proto.Message.InteractiveMessage.fromObject({
    ...
    header: {
      title: cleanText(menu.title || "CHATHU MD V4", 60),
      subtitle: cleanText(menu.subtitle || "", 60),
      hasMediaAttachment: false,
    },
    nativeFlowMessage: {
      buttons,
