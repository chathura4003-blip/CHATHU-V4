...
// Build extra CTA-style buttons (URL link / phone call / copy code) from a
// menu's `extraButtons` array. Each entry is `{ type, text, url|phone|code }`.
// Returns native-flow button shapes ready to be merged with quick_reply
// buttons or the single_select trigger. WhatsApp renders cta_url and
// cta_call as link / phone-call buttons that don't generate a reply, so we
// don't have to plumb their taps through extractNativeFlowId.
function _buildCtaButtons(menu) {
  ...
  for (const b of menu.extraButtons) {
    ...
    } else if (type === "call" && (b.phone || b.phone_number)) {
      if (menu?.callButton === false |
          | process.env.CALL_BUTTON === "off") continue;
      const phone = String(b.phone || b.phone_number).replace(/[^\d+]/g, "");
      if (!phone) continue;
      out.push({
        name: "cta_call",
        buttonParamsJson: JSON.stringify({
