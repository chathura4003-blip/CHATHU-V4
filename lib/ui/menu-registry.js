...
// Pull the top-level CTA buttons (banner image + Visit Us / Call us pair)
// out of env. All optional — when nothing is set the menu sends without an
// image header and without CTA pills, so the fallback is exactly the
// pre-PR layout.
//
//   MENU_BANNER_URL  → header image shown above the body / buttons
//   MENU_WEBSITE_URL → "🔗 Visit Us"  CTA URL button (opens link)
//   OWNER_NUMBER     → "📞 Call us"   CTA Call button (dials owner)
function _menuExtras() {
  let ownerNumber = "";
  try { ownerNumber = String(require("../../config").OWNER_NUMBER || "").trim(); } catch (_) {}
  ...
  if (ownerNumber && /^\d{6,15}$/.test(ownerNumber.replace(/[^\d]/g, ""))) {
    extras.push({ type: "call", text: "📞 Call us", phone: ownerNumber });
  }
  return { image, extras };
}
