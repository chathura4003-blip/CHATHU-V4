...
// .btnshow — demonstrates the full mixed-button card layout (image header +
// CTA URL + CTA Call + quick_reply pills) used by the new advanced UI mode.
//
...
//   OWNER_NUMBER      — "📞 Call us"  CTA Call button (phone dialer)
//
...
module.exports = {
  ...
  async execute(sock, msg, from) {
    ...
    const ownerNumber = String(cfg.OWNER_NUMBER || "").replace(/[^\d+]/g, "");
    ...
    if (ownerNumber && /^\+?\d{6,15}$/.test(ownerNumber)) {
      interactiveButtons.push({
        name: "cta_call",
        buttonParamsJson: JSON.stringify({
          display_text: "📞 Call us",
          phone_number: ownerNumber.startsWith("+") ? ownerNumber : `+${ownerNumber}`,
        }),
      });
    }
