  ...
  async execute(sock, msg, from, args, cmdName, context) {
    ...
    if (!url && keyword) {
      ...
      try {
        ...
        // V4.2: when Button Mode is ON, mirror the result list into the
        // bot-wide UI system so list/button selection + new menu-state
        // numeric reply work. When OFF, only the legacy text list above
        // is sent and old numeric reply (storeSearchResults) handles
        // selection.
