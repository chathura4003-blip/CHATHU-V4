...
module.exports = {
  ...
  async execute(sock, msg, from, args, name, context = {}) {
    ...
    roleMenu.rememberNumericMapping(sender, { level: "top", items, role });
    ...
    try {
      await sendInteractiveTopLevel({ sock, from, msg, role, prefix, botName, items, quotedRef });
    } catch (err) {
  ...
  async sendCategorySubmenu(sock, msg, from, sender, categoryId, context = {}) {
    ...
    roleMenu.rememberNumericMapping(sender, { level: "category", categoryId, items, role });
    ...
    try {
      await sendInteractiveCategory({ sock, from, msg, role, prefix, botName, categoryId, items, quotedRef });
    } catch (err) {
