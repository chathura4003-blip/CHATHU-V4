  ...
  async execute(sock, msg, from, args, cmdName, context) {
    ...
    try {
      ...
      if (cmdName === "yts") {
        ...
        // Mirror the same results into the bot-wide UI system so a list /
        // button select (when the user's client supports them) and quoted
        // numeric replies routed through menu-state both work. Best-effort.
        // V4.2: only when Button Mode is ON.
