...
const playCommand = {
  ...
  async execute(sock, msg, from, args, cmdName) {
    ...
    try {
      ...
      // Bot-wide quality select with custom options that match the legacy
      // numeric reply layout (1: Audio, 2: Voice Note, 3: Document, 4:
      // Video). The legacy numeric handler still works because it routes
      // off the playSelection cache.
      try {
        await ui.sendQualityMenu(sock, from, video, {
          title: "🎚 PLAY ACTION",
          items: [
            { label: "🎵 Audio",      title: "Audio",      action: "quality:select:audio",    payload: video },
            { label: "🎙️ Voice Note", title: "Voice Note", action: "quality:select:audio",    payload: video },
